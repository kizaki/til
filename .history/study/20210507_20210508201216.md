...
- Amazon Linux 2 は標準でvimが使える
- AWS Systems Managerを使うとログインしなくてもEC2でコマンドを実行できる

## cloud-input-output.log
 - EC2インスタンスを起動する初期処理（cloud-init）

初期処理をした時のログを確認する
```
sudo less /var/log/cloud-input-output.log
```
ログファイルのリアルタイム確認
```
sudo tail -f -n 20 messages
```
### 参考URL

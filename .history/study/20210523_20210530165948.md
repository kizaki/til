    ...
    - SQLの種類
        - SQLはそれぞれの処理内容ごとに大きく３種類に分類できる
            - データ定義言語（DDL: Data Definition Language）
                ...
                - その他のDDL
                    - DROP文
                        - データベース（スキーマ）やテーブルを削除する構文
                        - DROP DATABASE {DB名};
                        - DROP TABLE {IF EXISTS}{テーブル名};
                    - ALTER文
                        - データベース（スキーマ）やテーブルの設定を変更する構文
                            - カラム追加／削除
                            - カラムの設定変更（カラム名や制約の変更など）
                            ```
                            ALTER TABLE user_table
                            ADD COLUMN phone_number INT(11) NULL AFTER first_name;
                            ```
            - データ操作言語（DML: Data Manipulation Language）
                ...
                - 範囲検索にはBETWEEN句
                ```
                SELECT * FROM user_table WHERE id BETWEEN 10 AND 20;
                ```
                - 複数検索にはIN句
                ```
                SELECT * FROM user_table WHERE blood_type IN('A, 'B', 'O');
                ```
                - 複数条件にはAND句／OR句
                ```
                WHERE id = 1 AND blood type = 'B';
                WHERE id = 1 OR blood type = 'AB';
                ```
                - UPDATE文
                ```
                UPDATE user SET blood_type = 'AB' WHERE id = 1;
                ```
            ...
            - カラムの制約
                ...
                - キー制約（PK, UK, FK）
                    ...
                    - UK(UNIQUE KEY)制約
                        ...
                        - １つのユーザが同じ商品を購入しようとするとエラーになる
                    - FK(FOREIGN KEY)制約
                        - 「外部キー」と呼ばれ、この制約を作成したカラムは他のテーブルのカラムを参照し、他のテーブルに存在しない値は保存できないくなる
                        - user_tableに存在しないuser_idでpayment_tableにレコードを作ろうとするとエラー
                        - 不明なユーザによる決済情報が作られることを防止できる
